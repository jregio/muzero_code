...
class TrainResults(object):
    ...
    def plot_total_loss(self, num_simulations):
        x_vals = np.arange(len(self.total_losses))
        plt.plot(x_vals, self.total_losses, label="Train Loss")
        plt.xlabel("Train Steps")
        plt.ylabel("Loss")
        plt.savefig(str.format('plot_total_loss_{sim}.png', sim=num_simulations))
        plt.close()

    def plot_individual_losses(self, num_simulations):
        x_vals = np.arange(len(self.total_losses))
        ...
        plt.ylabel("Losses")
        plt.legend()
        plt.savefig(str.format('plot_individual_losses_{sim}.png', sim=num_simulations))
        plt.close()

    def plot_policy_loss(self, num_simulations):
        x_vals = np.arange(len(self.total_losses))
        plt.plot(x_vals, self.policy_losses, label="Policy Loss")
        plt.xlabel("Train Steps")
        plt.ylabel("Losses")
        plt.legend()
        plt.savefig(str.format('plot_policy_loss_{sim}.png', sim=num_simulations))
        plt.close()
...
class TestResults(object):
    ...
    def plot_rewards(self, num_simulations):
        x_vals = np.arange(len(self.test_rewards))
        plt.plot(x_vals, self.test_rewards, label="Test Reward")
        plt.xlabel("Test Episodes")
        plt.ylabel("Reward")
        plt.savefig(str.format('plot_rewards_{sim}.png', sim=num_simulations))
        plt.close()
