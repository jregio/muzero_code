...
def update_weights(config, network, optimizer, batch, train_results):
    ...
    def loss():
        ...
        #     state = state_batch[i]
        #     representation, value, policy_logits = network.initial_model.call(state)
        #     representation_batch.append(representation)
        #     value_batch.append(value)
        #     policy_batch.append(policy_logits)
        # representation_batch = tf.convert_to_tensor(representation_batch)
        # value_batch = tf.convert_to_tensor(value_batch)
        # policy_batch = tf.convert_to_tensor(policy_batch)

        representation_batch, value_batch, policy_batch = network.initial_model.call(state_batch)
        ...
        value_loss = tf.math.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(target_value_batch, value_batch[0]), axis=0)
        value_loss *= 0.25 # scale value loss

        policy_loss = tf.keras.losses.CategoricalCrossentropy()(target_policy_batch, network._softmax(policy_batch[0]))
        ...
        for actions_batch, targets_batch in zip(actions_batch, targets_recurrent_batch):
            ...
            ohe_actions_batch = np.array([action_to_one_hot(a, network.action_size)[0] for a in actions_batch])
            conditioned_batch = tf.concat([np.squeeze(representation_batch), ohe_actions_batch], axis=1)
            representation_batch, reward_batch, value_batch, policy_batch = network.recurrent_model.call(conditioned_batch)
            ...
            # Remember to scale value loss!
            # Add to total losses
            value_loss = tf.math.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(target_value_batch, value_batch), axis=0)
            value_loss *= 0.25 # scale value loss

            policy_loss = tf.keras.losses.CategoricalCrossentropy()(target_policy_batch, network._softmax(policy_batch))

            reward_loss = MSE(np.array(target_reward_batch), np.squeeze(reward_batch))

            loss += (value_loss + policy_loss + reward_loss) / num_unroll_steps
        ...
        train_results.total_losses.append(loss.numpy().item())
        train_results.value_losses.append(total_value_loss.numpy().item())
        train_results.policy_losses.append(total_policy_loss.numpy().item())
        train_results.reward_losses.append(total_reward_loss.numpy().item())
        return loss
